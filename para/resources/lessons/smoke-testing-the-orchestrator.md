...
1. **The global symlink loads the main checkout, not your worktree.**
   ...
   `pi_extensions/orchestrator/`. A plain `pi` run therefore loads the main
   checkout's code, so testing a branch's changes in a worktree requires loading
   that copy explicitly: `pi -e "$WT/pi_extensions/orchestrator/index.ts"
...
--no-extensions -e "$WT/pi_extensions/orchestrator/index.ts" ...` from a
