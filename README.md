...
The self-hosted **development environment** for [Balaur](https://github.com/alexradunet/balaur): the NixOS configuration that provisions the development host and the project-local Pi extensions that support remote development over NetBird. This repository is intentionally separate from the Balaur application repository so that the application (a static, dependency-free site) stays decoupled from the machine and tooling that develop it.
...
nixos_dev_env/                          NixOS system configuration (flake + modules)
  ...
  configuration.nix                     NetBird, Caddy HTTPS, balaur-dev service, user, locales
...
extensions/                             Pi package conventional directory (auto-discovered)
  balaur-netbird/                       Closed, TUI-confirmed NetBird Cloud Pi extension
...
The NixOS `balaur-dev` service runs the application's dependency-free development server from the application checkout, not from this repository:

```
WorkingDirectory = /home/balaur/projects/balaur
ExecStart         = ${nodejs_24}/bin/node scripts/balaur-dev.mjs
```

So the two repositories are checked out side by side on the development host:

```
/home/balaur/projects/balaur          application checkout (alexradunet/balaur)
/home/balaur/projects/balaur-dev-os   environment checkout (this repository)
```
...
```bash
cd /home/balaur/projects/balaur-dev-os
sudo nixos-rebuild switch --flake ./nixos_dev_env
```

This installs Pi and Herdr system-wide, starts NetBird, Caddy, and the `balaur-dev` live-reload service, and creates the protected `/etc/balaur/netbird.env` file (empty). Update Pi and Herdr pins with:
...
The `herdr_agent` extension reads worker roles from the application checkout's `.pi/agents/*.md` and skills from `.pi/skills/`; only the orchestration mechanism lives here. See `docs/adr/0003-netbird-pi-extension.md` and `extensions/balaur-netbird/README.md` for the security model.

## NetBird service-user credential

The NetBird extension reads its Personal Access Token on demand from `/etc/balaur/netbird.env`, a non-symlink regular file owned by `root:balaur-secrets` with mode `0640` that the NixOS configuration creates empty. A human operator creates a dedicated NetBird **service user** with the **Network Admin** role, creates a PAT for it, and installs the token with `sudoedit`. The token is never placed in this repository, the Nix store, a systemd environment, an issue, a chat, or a Pi session. See `extensions/balaur-netbird/README.md` for the full rotation procedure.
...
node --check extensions/balaur-netbird/*.mjs
node --check extensions/herdr-agents/*.js
```

The `herdr-agents` role-parser suite includes three project-integration assertions that check the application repository's real `.pi/agents` roles and `.pi/skills`. They run automatically when the application checkout is a sibling (`../balaur`) or when `BALAUR_APP_ROOT` points at it, and skip otherwise so this suite stays self-contained.
