...
- The token is read on demand from `/etc/nixpi/netbird.env`. It is never stored in the repository, Nix store, systemd environment, Pi session entries, tool output, dashboard, or logs.
- The config reader rejects symlinks and requires a regular file owned by `root:nixpi-secrets` with mode `0640`. Only one non-empty `NETBIRD_API_TOKEN` assignment is accepted; `export`, duplicate keys, and unknown keys are rejected.
...
Apply the NixOS configuration first; it creates the `nixpi-secrets` group, `/etc/nixpi`, and an empty protected token file. Reconnect the NetBird SSH session afterward so `nixpi` receives its new supplementary group:

```bash
sudo nixos-rebuild switch --flake ./nixos_dev_env
sudo stat -c '%U %G %a %n' /etc/nixpi /etc/nixpi/netbird.env
```
...
```bash
sudoedit /etc/nixpi/netbird.env
sudo chown root:nixpi-secrets /etc/nixpi/netbird.env
sudo chmod 0640 /etc/nixpi/netbird.env
```
...
node --check extensions/nixpi-netbird/config.mjs
node --check extensions/nixpi-netbird/client.mjs
node --check extensions/nixpi-netbird/contracts.mjs
node --check extensions/nixpi-netbird/mutations.mjs
node --check extensions/nixpi-netbird/presenters.mjs
```
